...
/**
 * this is the minimax function implement by myself
 * function maxValue() and minValue()
 * it has bugs when the difficult level higher than 1
  */

public class TicTacToeMinimaxByMe {
    ...
    public static int maxValue(char[][] state, int depth) {
        ...
        for (int[] validMove : validMoves) {
            int i = validMove[0];
            int j = validMove[1];
            v = Math.max(v, minValue(new char[i][j], depth));
        }
        return (int) v;
    }
    ...
    public static int minValue(char[][] state, int depth) {
        ...
        for (int[] validMove : validMoves) {
            int i = validMove[0];
            int j = validMove[1];
            v = Math.max(v, maxValue(new char[i][j], depth));
        }
        return (int) v;
    }
